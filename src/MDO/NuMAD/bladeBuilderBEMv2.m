%Blade Builder
function [blade]=bladeBuilderBEMv2(inputs,parentFolder,airfoils,filename)
...
for j=1:height(inputs)
    blade.percentthick(j)=100*max(blade.stations(table2array(inputs(j,7))).airfoil.coordinates(:,2));
end
...
afdb = [blade.stations.airfoil];
    ...
    blade.ispan      = (0:0.02:1)*table2array(inputs(end,1));%jjm: specify for some desired output interval
