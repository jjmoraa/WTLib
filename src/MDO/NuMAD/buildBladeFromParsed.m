...
blade.percentthick= geometryVec.percentthick; %this has to change later
...
%% Modify thickness
for j=1:length(blade.span)
    blade.percentthick(j)=2*100*max(blade.stations(geometryVec.afID(j)).airfoil.coordinates(:,2));
end
...
%% Add components
% add spar cap details
blade.sparcapwidth  = componentsVec(end).sparcapwidth;
blade.leband        = componentsVec(end).leband;
blade.teband        = componentsVec(end).teband;
% ble: <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
blade.sparcapoffset = componentsVec(end).sparcapoffset;
if isnan(blade.sparcapoffset), blade.sparcapoffset = 0; end

for k = 1:(length(componentsVec)-1)
    blade.addComponent(componentsVec(k));fprintf('%s \n',componentsVec(k).name)
end
