function [relwnds,aind,apind,amids,apmids,cpvector,ctvector,rmids,cmvector,cpmids,...
    ctmids,cmmids,ocp,oct,ocm,dT,dQ,totalpwr,mass,moment,blade,D,R,F,L,aerodyn]...
    =analysis_blocks_v2_03...
    (caseName,inputs,hubRad,parentFolder,airfoils,numel,results_folder)
...
[blade]=bladeBuilderBEMv2_03(inputs,parentFolder,airfoils,[parentFolder,'\Inputs\15MW_v2.xlsx'],numel);
    ...
    % Create table
    aerodyn = table(span'-hubRad, zeros(numel+1,1),  zeros(numel+1,1),  zeros(numel+1,1), twistVector', chordVector', airfoilno',  zeros(numel+1,1),  zeros(numel+1,1),  zeros(numel+1,1));
    ...
    movefile('*.inp', results_folder);
    movefile('*.out', results_folder);
    movefile('*.ech', results_folder);
    movefile('*.yaml', results_folder);
