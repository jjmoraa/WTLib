...
%% reference case run
    ...
    plottingCase(referenceCase,parentFolder,'referenceCase')
...
%Plotting all the reference case plots
    plottingCase(referenceCase,parentFolder,'crefJamiesonsCase')
...
for a_in=32
    for n_in=nbounds(1):nbounds(2):nbounds(3) %concave <1 convex
        for p_in=pbounds(1):pbounds(2):pbounds(3)
            ...
            jamiesonsCase(a_in,n_in,p_in).rmids=rmids;
            jamiesonsCase(a_in,n_in,p_in).hubRad=hubRad;
     ...
     %add case to results table
                newRow={a_in/100,n_in/100,p_in/100,a,ap,ocp,oct,ocm,totalpwr/crefjamiesonsCase.totalpwr...
                    ...
                    ,jamiesonsCase(a_in,n_in,p_in).blade.ispan(end)/crefjamiesonsCase.blade.span(end),D(1,end)/crefjamiesonsCase.D(1,end)};
...
for i = 2:size(jamiesonsCase(a_in,nbounds(3),pbounds(3)).blade.secprops.data,2)
    subplot(5, 5, i);
...
%Yeah I know, its a bit iffy to code it this way but im lazy
            ...
            [jinputs, ajam, apjam, jamchord, jamtwist, newhubRad]...
                =jamieson_v2_02_momentmatch(crefjamiesonsCase,crefmoment,wndspeed,a_in,...
