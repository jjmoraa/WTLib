function [aind,apind,amids,apmids,cpvector,ctvector,cmvector,cpmids,ctmids,cmmids,ocp,oct,ocm,dT,dQ,totalpwr,moment,blade,D,R,F,L,Ks]=analysis_blocks(inputs,hubRad,parentFolder,airfoils,numel)
    
    [blade]=bladeBuilderBEMv2_02(inputs,parentFolder,airfoils,[parentFolder,'\Inputs\15MW.xlsx'],numel);
    blade.updateBlade
    ...
    for j=1:height(inputs) %JJM: Here I would do length(blade.ispan)
        r = double(inputs{j,"span (r) [m]"})+hubRad; %JJM: Here blade.ispan(j) instead
