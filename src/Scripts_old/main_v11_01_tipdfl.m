...
tic
    ...
    plottingCase(referenceCase,parentFolder,'tip_dfl','referenceCase')
toc
%% jamiesons fit
...
tic
    ...
    saveas(fig,[parentFolder,'/Results/tip_dfl/','RefCase','equivalent fit.jpg'])
...
%This saves all the variables of this reference case
    crefjamiesonsCase =...
    variable_saving(1,1,1,a,ap,amids,apmids,ocp,oct,ocm,cpmids,ctmids,...
    cmmids,dT,dQ,totalpwr,crefmoment,crefmass,blade,rmids,crefnewhubRad,D,R,F,L,Ks,[parentFolder,'\Results'],'crefjamiesonsCase');
...
grid_points=JamiesonsBoundGenerator(fitcurve.a,fitcurve.n,fitcurve.p,9,fitcurve.p,5,25);
% n_points=linspace(2,4,10);
% p_points=linspace(0.01,0.5,3);

% [X, Y] = meshgrid(n_points, p_points);  % Create the grid
...
powertable_tipdfl = table([], [], [], [], [], [], [], [], [], [], [], [], [], [], 'VariableNames', {'A','n','p','a','ap','ocp','oct','ocm','total power','cpR^2','blade span', 'tip deflection','moment','mass'});
...
%corrected reference plots
    ...
    saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','BEM Parameters both references.jpg'])    
    ...
    saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','AD Calculations both references.jpg'])
    ...
    saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','Summary of performance calculations both references.jpg'])
    ...
    saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','Deflection both references.jpg'])
    %legend('dT','dQ','Location','Southwest')
...
foldername = [parentFolder, '/Results/tip_dfl/', sprintf('%s_%s/', scriptName, currentDate)]; % Create the filename
if ~isfolder(foldername)
    mkdir(foldername);
end
crefjamiesonsCaseTipdfl=crefjamiesonsCase;
crefjinputstipdfl=crefjinputs;
save(foldername, 'powertable_tipdfl','referenceCase','crefjamiesonsCaseTipdfl','crefjinputstipdfl'); % Save the variable
...
%% postpro
    %Heatmaps
fig = plotHeatmapFromTable(powertable_tipdfl, 'n','p', 'total power');
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','totalpowerjheatmap.jpg'])
fig = plotHeatmapFromTable(powertable_tipdfl, 'n','p', 'blade span');
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','bladespanjheatmap.jpg'])
fig = plotHeatmapFromTable(powertable_tipdfl, 'n','p', 'cpR^2');
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','cpr2heatmap.jpg'])
fig = plotHeatmapFromTable(powertable_tipdfl, 'n','p', 'tip deflection');
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','tipdflheatmap.jpg'])
fig = plotHeatmapFromTable(powertable_tipdfl, 'n','p', 'moment');
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','momentheatmap.jpg'])

    %surface plots
fig=plotSurfaceFromTable(powertable_tipdfl, 'n','p', 'total power');
fig.Position = [100, 100, 800, 600];  % [left, bottom, width, height]
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','totalpowerjsurf.jpg'])
fig=plotSurfaceFromTable(powertable_tipdfl, 'n','p', 'blade span');
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','radiusjsurf.jpg'])
fig=plotSurfaceFromTable(powertable_tipdfl, 'n','p', 'cpR^2');
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','cpr2surf.jpg'])
fig=plotSurfaceFromTable(powertable_tipdfl, 'n','p', 'moment');
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','tipdfljsurf.jpg'])
...
[fig,modpowertable,paretoX, paretoY, idx]=ParetoFront_v2(powertable_tipdfl,'cpR^2','tip deflection'); %max, min
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','ParetoCpr2tipdfl.jpg'])
fig=Important_plotter(idx,modpowertable,fitcurve.a,fitcurve.n,fitcurve.p);
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','ParetocurvesCpr2tipdfl.jpg'])

[fig,modpowertable,paretoX, paretoY, idx, globalidx]=ParetoFront_v3(powertable_tipdfl,'cpR^2','moment','mass'); %max, min
...
[fig,modpowertable,paretoX, paretoY, idx, globalidx]=ParetoFront_v3(powertable_tipdfl,'cpR^2','blade span','mass'); %max, min
saveas(fig,[parentFolder,'/Results/root_moment/','CRefCase','ParetoCpr2tipdfl.jpg'])
fig=Important_plotter(idx,modpowertable,fitcurve.a,fitcurve.n,fitcurve.p);
saveas(fig,[parentFolder,'/Results/root_moment/','CRefCase','ParetocurvesCpr2bladespanmass.jpg'])

[fig,modpowertable,paretoX, paretoY, idx]=ParetoFront_v2(powertable_tipdfl,'cpR^2','blade span'); %max, min
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','ParetoCpr2span.jpg'])
fig=Important_plotter(idx,modpowertable,fitcurve.a,fitcurve.n,fitcurve.p);
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','ParetocurvesCpr2span.jpg'])

[fig,modpowertable,paretoX, paretoY, idx]=ParetoFront_v2(powertable_tipdfl,'blade span','tip deflection'); %max, min
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','Paretospantipdfl.jpg'])
fig=Important_plotter(idx,modpowertable,fitcurve.a,fitcurve.n,fitcurve.p);
saveas(fig,[parentFolder,'/Results/tip_dfl/','CRefCase','Paretocurvesspantipdfl.jpg'])


%% comparisson plots
figure('Name', 'PreComp Analysis Results Jamiesons');
for i = 2:size(jamiesonsCase(index).blade.secprops.data, 2)
    subplot(5, 5, i);
    data = [referenceCase.blade.secprops.data]; % Concatenate results to form 'data'
    plot(data(:, 1), data(:, i), 'r-*');
    hold on
        for index=1:length(grid_points) %concave <1 convex
                data = [jamiesonsCase(index).blade.secprops.data]; % Concatenate results to form 'data'
                plot(data(:, 1), data(:, i), 'b-o');
                hold on
        end
    xlabel(strrep(jamiesonsCase(index).blade.secprops.labels{1}, '_', '\_'));
    ylabel(strrep(jamiesonsCase(index).blade.secprops.labels{i}, '_', '\_'));
    hold off
end
...
function [jinputs,ajam,apjam,jamchord,jamtwist,newhubRad,relWind,a,ap,amids,...
    ...
    if flb * fhb >= 0
        error('f(lb) and f(hb) must have opposite signs.');
    end
