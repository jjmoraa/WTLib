%Blade Builder
function [blade]=bladeBuilderBEM(inputs,parentFolder,airfoils,filename)
...
blade.percentthick=100*ones(height(inputs),1);
...
afdb = [blade.stations.airfoil];
    ...
    blade.ispan      = (0:0.05:1)*table2array(inputs(end,1));%fixate from some desired output interval
    %blade.ispan      = num(xls.geom.datarow1:end, xls.geom.ispan);%fixate from some desired output interval
