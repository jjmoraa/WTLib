...
[inputs,tsr,airfoils]=scriptInit(parentFolder);
...
%% reference case run
    ...
    [fitcurve, gof]=jamiesonsFit(referenceCase,6);
    ...
    for n_in=10:10:100 %concave <1 convex
        for p_in=10:10:100
            ...
            %Dstory=struct;
                ...
                %start of rootfinding procedure
            % step=1;
            % proposedR=1.5*(crefjamiesonsCase.blade.ispan(end)+crefnewhubRad);
            % tolerance=0.01;
            % indicator=tolerance+1;
            % %l=1;
            % proposedRnew=(crefjamiesonsCase.blade.ispan(end)+crefnewhubRad);
            %     while indicator>tolerance
            %     %for l=1:6
            %         proposedR=proposedRnew;
            %         [jinputs,ajam,apjam,jamchord,jamtwist,newhubRad]...
            %                     =jamieson_v2_02_tipdfl(crefjamiesonsCase,crefmoment,wndspeed,a_in/100,...
            %                     n_in/100,p_in/100,crefnewhubRad,3,jinputs,parentFolder,airfoils,proposedR);
            % 
            %         [jinputsph,ajamph,apjamph,jamchordph,jamtwistph,newhubRadph]...
            %                     =jamieson_v2_02_tipdfl(crefjamiesonsCase,crefmoment,wndspeed,a_in/100,...
            %                     n_in/100,p_in/100,crefnewhubRad,3,jinputs,parentFolder,airfoils,proposedR+step);
            % 
            %         [jinputsmh,ajammh,apjammh,jamchordmh,jamtwistmh,newhubRadmh]...
            %                     =jamieson_v2_02_tipdfl(crefjamiesonsCase,crefmoment,wndspeed,a_in/100,...
            %                     n_in/100,p_in/100,crefnewhubRad,3,jinputs,parentFolder,airfoils,proposedR-step);
            % 
            %         %topSecProps=struct;
            ...
            %                         ~,~,~,~,~,~,~,~,~,~,Dph,~,~,~,~]...
            %                         =analysis_blocks_v2_02...
            %                         (jinputsph,newhubRadph,parentFolder,airfoils,numel);
            % 
            %                     [~,~,~,~,~,~,~,~,~,~,...
            %                         ~,~,~,~,~,~,~,~,~,~,Dmh,~,~,~,~]...
            %                         =analysis_blocks_v2_02...
            %                         (jinputsmh,newhubRadmh,parentFolder,airfoils,numel);
            %                     %topSecProps(count).secprops=blade.secprops.data;
            %                     %ispans(count).ispan=blade.ispan;
            %                     %Dstory(count).D=D;
            % 
            %         dflprime=(Dph(2,end)-Dmh(2,end))/(2*step);
            %         proposedRnew=proposedR-(D(2,end)-referenceCase(1).D(2,end))/dflprime;
            %         %l=l+1;
            % 
...
function [jinputs,ajam,apjam,jamchord,jamtwist,newhubRad,relWind,a,ap,amids,...
        ...
        step=1;
%         %proposedR=1.5*(referenceCase.blade.ispan(end)+hubRad);
%         tolerance=0.01;
%         indicator=tolerance+1;
% 
%         proposedRnew=(crefjamiesonsCase.blade.ispan(end)+crefnewhubRad);
...
% 
%             [jinputs,ajam,apjam,jamchord,jamtwist,newhubRad]...
%                         =jamieson_v2_02_tipdfl(crefjamiesonsCase,crefmoment,wndspeed,a_in,...
%                         n_in,p_in,crefnewhubRad,numblades,jinputs,parentFolder,airfoils,proposedR);
% 
%             [jinputsph,ajamph,apjamph,jamchordph,jamtwistph,newhubRadph]...
%                         =jamieson_v2_02_tipdfl(crefjamiesonsCase,crefmoment,wndspeed,a_in,...
%                         n_in,p_in,crefnewhubRad,numblades,jinputs,parentFolder,airfoils,proposedR+step);
% 
%             [jinputsmh,ajammh,apjammh,jamchordmh,jamtwistmh,newhubRadmh]...
%                         =jamieson_v2_02_tipdfl(crefjamiesonsCase,crefmoment,wndspeed,a_in,...
%                         n_in,p_in,crefnewhubRad,numblades,jinputs,parentFolder,airfoils,proposedR-step);
...
% 
%             dflprime=(Dph(2,end)-Dmh(2,end))/(2*step);
%             proposedRnew=proposedR-(D(2,end)-crefjamiesonsCase(1).D(2,end))/dflprime;
% 
%             indicator=abs(proposedRnew-proposedR);
% end
...
while current_guess_index <= length(initial_guesses)
    ...
    while indicator > tolerance
        ...
        [relWind, a, ap, amids, apmids, cpvector, ctvector, rmids, cmvector, cpmids, ...
            ctmids, cmmids, ocp, oct, ocm, dT, dQ, totalpwr, crefmoment, blade, D, R, F, L, Ks] = ...
            analysis_blocks_v2_03('referenceCase',jinputs, newhubRad, parentFolder, airfoils, numel);

        % Analysis blocks for + step
        [~, ~, ~, ~, ~, ~, ~, ~, ~, ~,...
            ~, ~, ~, ~, ~, ~, ~, ~, ~, ~,...
             Dph, ~, ~, ~, ~] = ...
            analysis_blocks_v2_03('referenceCase',jinputsph, newhubRadph, parentFolder, airfoils, numel);

        % Analysis blocks for - step
        [~, ~, ~, ~, ~, ~, ~, ~, ~, ~,...
            ~, ~, ~, ~, ~, ~, ~, ~, ~, ~,...
             Dmh, ~, ~, ~, ~] = ...
            analysis_blocks_v2_03('referenceCase',jinputsmh, newhubRadmh, parentFolder, airfoils, numel);
