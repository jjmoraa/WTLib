function [inputs,a,ap,chord,twist,newhubRad]=...
    jamieson_v2_02_tipdfl...
    (referenceCase,refmoment,wndspeed,A,n,p,hubRad,blades,inputs,parentFolder,airfoils,R)
...
%loop for each point of the rotor
aoa=zeros(1,length(span));
cl=zeros(1,length(span));
cd=zeros(1,length(span));
chord=zeros(1,length(span));
...
dummy(:,1)=span(1:end)-newhubRad;dummy(1,1)=0;
dummy(:,5)=twist(1:end);
dummy(:,6)=chord(1:end);
dummy(:,7)=airfoilno(1:end);
